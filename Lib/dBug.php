...
class dBug {
    ...
    public static $bufferOutput = false;
	...
	//Abbey's output wrapper: allows console output or internal buffering
	
	function output($text){
	   if(dBug::$bufferOutput){
	       $this->buffer .= $text;
	   }else{
	       echo($text);
	   }
	...
	// Jon's Buffer functions (Used in jon' class.xxxx.php files)
	
	function addToOutputBuffer($somecontent) {
		$this->buffer = $this->buffer . $somecontent;
	}
	...
	function checkString($start=0, $end=1000) {		
		$returnthis = '';
		$thischarstring = $this->debugtext;
		$returnthis .= '<table border="1"><tr><td>index</td><td>character</td><td>dec</td><td>hex</td></tr>';
					for($b=$start;$b<strlen($thischarstring) && $b < $end;$b++) {
						$returnthis .= '<tr>';
							$returnthis .= '<td>' . $b . '</td>';
							$returnthis .= '<td>' . substr($thischarstring, $b, 1) . '</td>';
							$returnthis .= '<td>' . ord(substr($thischarstring, $b, 1)) . '</td>';
							$returnthis .= '<td>' .  str_pad  ( dechex(ord(substr($thischarstring, $b, 1))), 
											2, 
											'0', 
											STR_PAD_LEFT) . '</td>';
						$returnthis .= '</tr>';
	...
	function varIsArray($var) {
		...
		else $this->output( "<tr><td>".$this->error("array").$this->closeTDRow());
	...
	function varIsObject($var) {
		...
		else $this->output( "<tr><td>".$this->error("object").$this->closeTDRow());
		array_pop($this->arrHistory);
		$this->output( "</table>");
	}
	...
	function initJSandCSS() {
		$this->output('
			...
			/* code modified from ColdFusions cfdump code */
